...
void status_timer_callback(TimerHandle_t xTimer)
{
    ...
    ESP_LOGI(TAG, "Extra events:     %lu", stats.extra_count);
    ESP_LOGI(TAG, "═══════════════════════");
...
// Extra timer callback for load testing
void extra_callback(TimerHandle_t xTimer)
{
    uint32_t id = (uint32_t)pvTimerGetTimerID(xTimer);
    stats.extra_count++;
    ESP_LOGI(TAG, "Extra Timer %lu fired (total: %lu)", id, stats.extra_count);
}
...
void app_main(void)
{
    ...
    // Create extra timers for load testing
    ESP_LOGI(TAG, "Creating extra timers for load testing...");
    for (int i = 0; i < 10; i++)
    {
        ...
        if (extra_timer != NULL)
        {
            ...
            ESP_LOGI(TAG, "Extra timer %d created and started (period: %lums)", i, 100 + i * 50);
        }
        else
        {
            ESP_LOGW(TAG, "Failed to create extra timer %d", i);
        }
    }
