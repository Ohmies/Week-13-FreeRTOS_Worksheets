...
// Event Correlation Metrics Functions
        ...
        if (sensor_bits != 0)
        {
            ...
            for (int p = 0; p < NUM_PATTERNS; p++)
            {
                ...
                if (pattern->required_events[event_index] == 0)
                {
                    ...
                    // Calculate correlation strength (events matched / events required)
                    int required_events = 0;
                    for (int e = 0; e < 4 && pattern->required_events[e] != 0; e++)
                    {
                        required_events++;
                    }
                    analytics.correlation_strength[p] = (float)required_events / required_events; // Perfect match = 1.0
...
void motion_sensor_task(void *pvParameters)
{
    ...
    while (1)
    {
        ...
        { // 15% chance per cycle
            ...
            { // 60% chance of presence confirmation
    ...
    while (1)
    {
        ...
        if (adaptive_params.learning_mode)
        {
            ...
            for (int h = 0; h < EVENT_HISTORY_SIZE; h++)
            {
                ...
                if ((current_time - record->timestamp) < 300000000)
                { // Last 5 minutes
...
void status_monitor_task(void *pvParameters)
{
    ...
    while (1)
    {
        ...
        ESP_LOGI(TAG, "False Positives: %lu", analytics.false_positives);
