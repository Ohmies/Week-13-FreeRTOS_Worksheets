...
void stack_monitor_task(void *pvParameters)
{
    ...
    UBaseType_t previous_remaining[4] = {0}; // For dynamic monitoring
    ...
    while (1)
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            if (tasks[i] != NULL)
            {
                ...
                // Dynamic monitoring
                dynamic_stack_monitor(tasks[i], task_names[i], &previous_remaining[i]);
...
void recursive_function(int depth, char *buffer)
{
    ...
    if (depth < 20)
    { // Limit recursion to prevent complete stack overflow
        vTaskDelay(pdMS_TO_TICKS(500));
        recursive_function(depth + 1, buffer);
    }
}

void dynamic_stack_monitor(TaskHandle_t task_handle, const char *task_name, UBaseType_t *previous_remaining)
{
